...
import { LandingPageOriginComponent } from './LandingPageOrigin';

const { waitFor } = testingLibrary;

describe('LandingPageOrigin', () => {
  it('renders the Fallback page on error', async () => {
    const errorMessage = 'LandingPageOrigin failed';
    let e = new Error(errorMessage);
    e.type = 'error';
    e.name = 'Test';

    const { getByText } = render(
      <LandingPageOriginComponent pageAssetsData={null} inProgress={false} error={e} />
    );
  ...
  it('renders given pageAssetsData', async () => {
    ...
    const { getByText } = render(
      <LandingPageOriginComponent
        pageAssetsData={{ LandingPageOrigin: { data } }}
        inProgress={false}
        error={null}
      />
    );
